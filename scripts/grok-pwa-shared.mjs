...
function detectCustomOgCard(cwd = process.cwd(), site = {}) {
  ...
  return detectCustomOgCardFromSite(site);
}

function detectCustomOgCardFromSite(site = {}) {
...
export function resolveOgCardAsset(site = {}, cwd = process.cwd()) {
  if (!cwd) {
    // No explicit workspace: trust a bake that already saw the file (the
    // Vercel runtime has no public/ to consult).
    return detectCustomOgCardFromSite(site) ? String(site.image ?? "").trim() || "/og.jpg" : "";
  }
  return (
...
export function normalizeHeadContext(ctx = {}) {
  // Filesystem discovery only happens when a cwd is EXPLICITLY provided.
  // Defaulting to process.cwd() here makes the pure head-injection functions
  // depend on whatever directory the caller happens to run from (tests and
  // other tools would pick up an unrelated src/lib/og/site.json). All
  // production call sites (vite plugin, dev middleware) pass an explicit cwd.
  const cwd = typeof ctx.cwd === "string" && ctx.cwd ? ctx.cwd : null;
